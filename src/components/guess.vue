<template>
	<section class="guess">
		...
		<ul class="loadmore-body">
			<li v-for="(value) in list" class="card">
				<a href=""><img :src="value.img[0]" alt=""></a>
				<p>{{value.title}}</p>
				<p>￥{{value.price}}</p>
			</li>
		</ul>
		<div class="card"><input @click="getMore(pageCode)" type="button" class="button" value="加载更多"></div>
	</section>
</template>
...
export default {
  ...
  methods:{
  	...
  	getMore(page){
  		...
  		this.pageCode++;
  		console.log(this.list,this.pageCode)
  	}
  }
}
</script>
